...
async def check_and_use(uid: str, n: int, is_upload: bool = True) -> bool:
    ...
    daily_traffic[today_key] += n
    if is_upload:
        traffic_split["upload_bytes"] += n
    else:
        traffic_split["download_bytes"] += n
    try:
        from main import track_user_usage_for_link
        asyncio.create_task(track_user_usage_for_link(uid, n))
    except Exception:
        pass
    return True
...
async def relay_ws_to_tcp(ws: WebSocket, writer: asyncio.StreamWriter, conn_id: str, uid: str):
    try:
        while True:
            ...
            if not data:
                continue
            if not await check_and_use(uid, len(data), is_upload=True):
...
async def relay_tcp_to_ws(ws: WebSocket, reader: asyncio.StreamReader, conn_id: str, uid: str):
    ...
    try:
        while True:
            ...
            if not data:
                break
            if not await check_and_use(uid, len(data), is_upload=False):
                await ws.close(code=1008, reason="quota/disabled/unknown")
                break
            await throttle(uid, len(data))
            connections[conn_id]["bytes"] += len(data)
...
async def websocket_tunnel(ws: WebSocket, uuid: str):
    ...
    try:
        ...
        if BLOCK_DOMAINS_ENABLED and is_domain_blocked(address):
            logger.warning(f"🚫 WS blocked domain={address} uuid={uuid[:8]}…")
            await ws.close(code=1008, reason="domain blocked")
            return
        if BLOCK_ADS_ENABLED and is_domain_ads(address):
            logger.warning(f"🚫 WS blocked ads domain={address} uuid={uuid[:8]}…")
            await ws.close(code=1008, reason="ads blocked")
            return

        if not await check_and_use(uuid, len(first_chunk), is_upload=True):
