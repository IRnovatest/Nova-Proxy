...
class _QuotaGate:
    ...
    def __init__(self, uuid: str, is_upload: bool = True):
        self.uuid = uuid
    ...
    async def add(self, nbytes: int) -> bool:
        ...
        if self.pending >= self.batch_bytes or elapsed >= QUOTA_CHECK_INTERVAL:
            ...
            self.last_check = now
            self.ok = await check_and_use(self.uuid, flush, is_upload=self.is_upload)
            return self.ok
        return True
    ...
    async def flush(self) -> bool:
        if self.pending:
            flush, self.pending = self.pending, 0
            self.ok = self.ok and await check_and_use(self.uuid, flush, is_upload=self.is_upload)
        return self.ok
...
async def _open_tcp_from_header(first_chunk: bytes):
    ...
    if BLOCK_ADS_ENABLED and is_domain_ads(address):
        raise HTTPException(status_code=403, detail="ads blocked")
...
async def _pump_tcp_to_queue(session_id: str, uuid: str, reader: asyncio.StreamReader, down_q: asyncio.Queue):
    first = True
    gate = _QuotaGate(uuid, is_upload=False)  # دانلینک → is_upload=False
    try:
        while True:
